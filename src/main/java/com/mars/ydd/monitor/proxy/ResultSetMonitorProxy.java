...
public class ResultSetMonitorProxy implements ResultSet {
	private final ResultSet resultSet;
	private final String sql;
	private final long executeSqlTime;
	private final MonitorDataProcessor monitorDataProcessor;
	private boolean isCountFinish = false;
	private final AtomicInteger retrieveRowCount = new AtomicInteger(0);
	...
	public long getExecuteSqlTime() {
		return executeSqlTime;
	}
	...
	public AtomicInteger getRetrieveRowCount() {
		return retrieveRowCount;
	}

	public ResultSetMonitorProxy(ResultSet resultSet, String sql, long executeSqlTime,
			MonitorDataProcessor monitorDataProcessor) {
	...
	public boolean next() throws SQLException {
		...
		if (result) {
			retrieveRowCount.incrementAndGet();
		}
		return result;
	}
	...
	public boolean relative(int rows) throws SQLException {
		...
		if (!isCountFinish) {
			if (result && rows >= 0) {
				retrieveRowCount.addAndGet(rows);
			} else {
				isCountFinish = true;
			}
		}
